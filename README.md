...
**Current scope:** this release only discovers and manages **lighting devices** — bulbs, light strips, wall switches (always treated as light loads, since a switch can only be wired to a light), and plugs. Plugs are treated as lights when the **Treat Plugs As Lights** setting is enabled, except for known dimmable plugs (for example, `P135`), which are always treated as lights regardless of that setting, since a dimmable plug can only meaningfully be used to dim a light. Other Kasa/Tapo device types (for example, plain on/off plugs/switches exposed as outlets, power-monitoring plugs, or sensors) are not yet published as child devices, though some groundwork for future device types already exists in the platform's discovery and caching layers. Support for additional device types is planned for future releases.
...
| **Treat Plugs As Lights** | When enabled, supported smart plugs are exposed as light entities instead of being ignored. Known dimmable plugs (for example, `P135`) are always exposed as lights regardless of this setting, since a dimmable plug can only meaningfully be used to dim a light. |
